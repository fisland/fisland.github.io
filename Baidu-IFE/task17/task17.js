...
function renderChart() {
    ...
    var source = chartData[pageState.nowGraTime][pageState.nowSelectCity];
    var length = Object.keys(source).length;
    var opsObj = getWidth(width,length);
    innerHTML += "<div class='title'>"+pageState.nowSelectCity+"市01-03月"+getTitle()+"空气质量报告</div>";
    for(var key in source){
        // innerHTML += "<div class='aqi-bar " + pageState.nowGraTime + "' style='height:" + source[key] + "px; width: " + opsObj.width +"px; left:" + (opsObj.left * i + opsObj.offsetLeft) + "px; background-color:" + colors[Math.floor(Math.random() * 11)] + "'></div>"
        innerHTML += "<div class='aqi-bar " + pageState.nowGraTime + "' style='height:" + source[key] + "px; width: " + opsObj.width +"px; left: "+(opsObj.left*i + opsObj.offsetLeft) + "px; background-color:"+colors[Math.floor(Math.random()*11)]+ "'></div>";
        innerHTML += "<div class='aqi-hint ' style='bottom: "+(source[key]+10)+"px; left:"+getHintLfeft(opsObj,i)+"px'>"+"<br/>[AQI]:"+source[key]+"</div>";
        i++;
    }
...
function initGraTimeForm() {
    ...
    for(var i = 0;i<radio.length;i++){
        (function (m) {
            addEventHandle(radio[m],'click',function() {
...
function initCitySelector() {
    ...
    selector.innerHTML = cityList;
    // 给select设置事件，当选项发生变化时调用函数citySelectChange
    addEventHandle(selector,"change",citySelectChange);
...
function initAqiChartData() {
    ...
    for (var key in aqiSourceData){
        ...
        for (var item in tempCity){
            countSum+=tempCity[item];
            daySum++;
            if ((new Date(item)).getDay() == 6){
                week++;
                weekSource["第"+week+"周"] = Math.floor(countSum/daySum);
                countSum = 0;
                daySum = 0;
            }
        ...
        for (var item in tempCity){
            ...
            if ((new Date(item)).getMonth() !== month){
                month++;
                monthSource["第"+month+"月"] = Math.floor(countSum/daySum);
                countSum = 0;
                daySum = 0;
            }
        }
        ...
        cityWeek[key] = weekSource;
        cityMonth[key] = monthSource;
    }
    //     处理好的数据存到 chartData 中
    chartData.day = aqiSourceData;
    chartData.week = cityWeek;
    chartData.month = cityMonth;
}
